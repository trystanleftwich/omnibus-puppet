...
# Ruby gems support.
Puppet::Type.type(:package).provide :puppet_gem, :parent => Puppet::Provider::Package do

  desc "A clone of the gem provider, but uses the full stack puppet embeded gem
  to install local to puppet.  If a URL is passed via `source`, then that
  URL is used as the remote gem repository; if a source is present but is
  not a valid URL, it will be interpreted as the path to a local gem file.
  ...
  command-line flags to be passed to the gem command.  These options should
  be specified as: a string (e.g. '--flag'), a hash (e.g. {'--flag' =>
  'value'}), or an array where each element is either a string or a hash."
